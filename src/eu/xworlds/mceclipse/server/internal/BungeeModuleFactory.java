 ...
 *
 */
public class BungeeModuleFactory extends ProjectModuleFactoryDelegate
{
    ...
    {
        BungeeModule moduleDelegate = null;
        IModule module = null;
        try
        {
            if (isValidPlugin(project))
            {
                moduleDelegate = new BungeeModule(project);
                module = createModule(project.getName(), project.getName(), "bungee.plugin", "1.0", project);
                moduleDelegate.initialize(module);
            }
            if (isValidLib(project))
            {
                moduleDelegate = new BungeeModule(project);
                module = createModule(project.getName(), project.getName(), "bungee.library", "1.0", project);
                moduleDelegate.initialize(module);
            }
        }
        catch (Exception e)
    ...
    protected boolean isValidPlugin(IProject project) {
        try {
            ...
            if (facetedProject == null)
                return false;
            IProjectFacet spigotPlugin = ProjectFacetsManager.getProjectFacet("bungee.plugin"); //$NON-NLS-1$
    ...
    protected boolean isValidLib(IProject project) {
        try {
            ...
            if (facetedProject == null)
                return false;
            IProjectFacet spigotLib = ProjectFacetsManager.getProjectFacet("bungee.lib"); //$NON-NLS-1$
